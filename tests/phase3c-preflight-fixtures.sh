...
export PHASE3C_FIXTURE_VERSION='154.0.8037.58'
export PHASE3C_FIXTURE_PREVIOUS_VERSION='154.0.8037.57'

cat > "$stub_dir/mkdir" <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
target="${@: -1}"
if [[ "${PHASE3C_FIXTURE_FAIL_ROOT_MKDIR:-0}" == 1 && "$(basename "$target")" == attempt-20260923-120000 && "$#" == 1 ]]; then
  exit 1
fi
/bin/mkdir "$@"
if [[ "${PHASE3C_FIXTURE_POST_CREATE_SYMLINK_SWAP:-0}" == 1 && "$(basename "$target")" == attempt-20260923-120000 && "$#" == 1 ]]; then
  /bin/rmdir "$target"
...
legacy_version="$versions/154.0.8037.57"
current_version="$versions/$PHASE3C_FIXTURE_VERSION"
mkdir -p "$source_app/Contents/MacOS" "$source_app/Contents/Resources" \
  ...
  "$current_version/Libraries" "$artifact_dir"
ln -s "$PHASE3C_FIXTURE_VERSION" "$versions/Current"
ln -s Versions/Current/Libraries "$framework/Libraries"
cat > "$source_app/Contents/Info.plist" <<'EOF'
<?xml version="1.0" encoding="UTF-8"?>
<!DOCTYPE plist PUBLIC "-//Apple//DTD PLIST 1.0//EN" "http://www.apple.com/DTDs/PropertyList-1.0.dtd">
<plist version="1.0"><dict><key>CFBundleShortVersionString</key><string>154.0.8037.58</string><key>CFBundleVersion</key><string>8037.58</string><key>CFBundleExecutable</key><string>Google Chrome</string></dict></plist>
EOF
printf '#!/bin/sh\nexit 0\n' > "$source_app/Contents/MacOS/Google Chrome"
printf 'legacy framework\n' > "$legacy_version/Google Chrome Framework"
...
printf 'baseline\n' > "$current_version/Libraries/libchrome.dylib"
mkdir -p "$current_version/Libraries/IwaKeyDistribution/nested"
printf 'nested baseline\n' > "$current_version/Libraries/IwaKeyDistribution/nested/file"
ln -s nested/file "$current_version/Libraries/IwaKeyDistribution/internal-link"
printf 'egl\n' > "$artifact_dir/libEGL.dylib"
printf 'gles\n' > "$artifact_dir/libGLESv2.dylib"
cat > "$artifact_dir/args.gn" <<'EOF'
target_os = "mac"
target_cpu = "x64"
EOF
fixture_egl_sha=$(/usr/bin/shasum -a 256 "$artifact_dir/libEGL.dylib" | awk '{print $1}')
fixture_gles_sha=$(/usr/bin/shasum -a 256 "$artifact_dir/libGLESv2.dylib" | awk '{print $1}')
fixture_gn_sha=$(/usr/bin/shasum -a 256 "$artifact_dir/args.gn" | awk '{print $1}')
cat > "$artifact_dir/ANGLE_RELEASE_MANIFEST" <<EOF
SCHEMA=angle-release-v1
ARTIFACT_SCHEMA=angle-artifact-v1
CHROME_VERSION=$PHASE3C_FIXTURE_VERSION
...
ARTIFACT_NAME=angle-macos-x86_64-chrome-$PHASE3C_FIXTURE_VERSION-angle-22222222-123456789
BUILD_RUN_ID=123456789
BUILT_AT_UTC=2026-09-23T12:00:00Z
GN_ARGS_SHA256=$fixture_gn_sha
EOF
/usr/bin/shasum -a 256 "$artifact_dir/ANGLE_RELEASE_MANIFEST" | awk '{print $1 "  ANGLE_RELEASE_MANIFEST"}' > "$artifact_dir/ANGLE_RELEASE_MANIFEST.sha256"
...
mkdir -p "$fixture/existing-output-root/attempt-20260923-120000" "$fixture/existing-results-root/attempt-20260923-120000"
mkdir "$fixture/existing-output-root/attempt-20260923-120000/existing-output.app" "$fixture/existing-results-root/attempt-20260923-120000/existing-results"
mkdir -p "$fixture/existing-root/attempt-20260923-120000"
expect_rejected_without_prepare "$preflight" --source-app "$source_app" --artifact-dir "$artifact_dir" --output-app "$fixture/existing-root/attempt-20260923-120000/output.app" --results-dir "$fixture/existing-root/attempt-20260923-120000/results"
expect_rejected_without_prepare "$preflight" --source-app "$source_app" --artifact-dir "$artifact_dir" --output-app "$fixture/existing-output-root/attempt-20260923-120000/existing-output.app" --results-dir "$fixture/existing-output-root/attempt-20260923-120000/new-results"
expect_rejected_without_prepare "$preflight" --source-app "$source_app" --artifact-dir "$artifact_dir" --output-app "$fixture/existing-results-root/attempt-20260923-120000/new-output.app" --results-dir "$fixture/existing-results-root/attempt-20260923-120000/existing-results"
expect_rejected_without_prepare "$preflight" --source-app "$source_app" --artifact-dir "$artifact_dir" --output-app /Applications/attempt-20260923-120000/phase3c-writable.app --results-dir /Applications/attempt-20260923-120000/phase3c-results
expect_rejected_without_prepare "$preflight" --source-app "$source_app" --artifact-dir "$artifact_dir" --output-app "$source_app/preflight-root/attempt-20260923-120000/output.app" --results-dir "$source_app/preflight-root/attempt-20260923-120000/results"
ln -s "$source_app" "$fixture/source-link.app"
mkdir -p "$fixture/link-root" "$fixture/hash-root" "$fixture/process-root"
expect_rejected_without_prepare "$preflight" --source-app "$fixture/source-link.app" --artifact-dir "$artifact_dir" --output-app "$fixture/link-root/attempt-20260923-120000/output.app" --results-dir "$fixture/link-root/attempt-20260923-120000/results"
expect_rejected_without_prepare env FORCE_HASH_MISMATCH=1 "$preflight" --source-app "$source_app" --artifact-dir "$artifact_dir" --output-app "$fixture/hash-root/attempt-20260923-120000/output.app" --results-dir "$fixture/hash-root/attempt-20260923-120000/results"
export PHASE3C_SOURCE_APP="$source_app"
expect_rejected_without_prepare env PHASE3C_SOURCE_PROCESS=1 "$preflight" --source-app "$source_app" --artifact-dir "$artifact_dir" --output-app "$fixture/process-root/attempt-20260923-120000/output.app" --results-dir "$fixture/process-root/attempt-20260923-120000/results"
...
expect_rejected_without_prepare "$preflight" --source-app "$source_app" --artifact-dir "$artifact_dir" --output-app "$fixture/missing-parent/attempt-20260923-120000/output.app" --results-dir "$fixture/missing-parent/attempt-20260923-120000/results"
expect_rejected_without_prepare "$preflight" --source-app "$source_app" --artifact-dir "$artifact_dir" --output-app "$fixture/symlink-parent/attempt-20260923-120000/output.app" --results-dir "$fixture/symlink-parent/attempt-20260923-120000/results"
expect_rejected_without_prepare "$preflight" --source-app "$source_app" --artifact-dir "$artifact_dir" --output-app "$fixture/mismatch-a/attempt-20260923-120000/output.app" --results-dir "$fixture/mismatch-b/attempt-20260923-120000/results"
expect_rejected_without_prepare "$preflight" --source-app "$source_app" --artifact-dir "$artifact_dir" --output-app "$fixture/lifecycle-parent/attempt-20260923-120000/nested/output.app" --results-dir "$fixture/lifecycle-parent/attempt-20260923-120000/results"
expect_rejected_without_prepare "$preflight" --source-app "$source_app" --artifact-dir "$artifact_dir" --output-app "$fixture/lifecycle-parent/attempt-20260923-120000/output.app" --results-dir "$fixture/lifecycle-parent/attempt-20260923-120000/output.app"
expect_rejected_without_prepare env PHASE3C_FIXTURE_FAIL_ROOT_MKDIR=1 "$preflight" --source-app "$source_app" --artifact-dir "$artifact_dir" --output-app "$fixture/root-mkdir-parent/attempt-20260923-120000/output.app" --results-dir "$fixture/root-mkdir-parent/attempt-20260923-120000/results"
test ! -e "$fixture/root-mkdir-parent/attempt-20260923-120000"
expect_rejected_without_prepare env PHASE3C_FIXTURE_FAIL_RESULTS_MKDIR=1 "$preflight" --source-app "$source_app" --artifact-dir "$artifact_dir" --output-app "$fixture/results-mkdir-parent/attempt-20260923-120000/output.app" --results-dir "$fixture/results-mkdir-parent/attempt-20260923-120000/results"
test ! -e "$fixture/results-mkdir-parent/attempt-20260923-120000/results"
expect_rejected_without_prepare env PHASE3C_FIXTURE_POST_CREATE_SYMLINK_SWAP=1 "$preflight" --source-app "$source_app" --artifact-dir "$artifact_dir" --output-app "$fixture/swap-parent/attempt-20260923-120000/output.app" --results-dir "$fixture/swap-parent/attempt-20260923-120000/results"
test ! -e "$fixture/swap-parent/attempt-20260923-120000/results"
expect_rejected_without_prepare "$preflight" --source-app "$source_app" --artifact-dir "$artifact_dir" --output-app "$fixture/lifecycle-parent/attempt-20260923-120000/../output.app" --results-dir "$fixture/lifecycle-parent/attempt-20260923-120000/results"
expect_rejected_without_prepare "$preflight" --source-app "$source_app" --artifact-dir "$artifact_dir" --output-app "$fixture/lifecycle-parent//attempt-20260923-120000/output.app" --results-dir "$fixture/lifecycle-parent//attempt-20260923-120000/results"
for retry_name in retry0 retry1 retry2 retry3 retry4 retry5 retry6 retry7 retry8 retry9 retry10 retry11; do
...
expect_rejected_without_prepare "$preflight" --source-app "$source_app" --artifact-dir "$artifact_dir" --output-app "$fixture/retry12/output.app" --results-dir "$fixture/retry12/results"
...
for release_case in manifest-tamper sidecar schema chrome-version dylib; do
  ...
  case "$release_case" in
    manifest-tamper)
      printf 'TAMPERED=yes\n' >> "$bad_artifact/ANGLE_RELEASE_MANIFEST" ;;
    sidecar)
      printf '%064d  ANGLE_RELEASE_MANIFEST\n' 0 > "$bad_artifact/ANGLE_RELEASE_MANIFEST.sha256" ;;
    schema)
      sed 's/^SCHEMA=.*/SCHEMA=angle-release-v0/' "$bad_artifact/ANGLE_RELEASE_MANIFEST" > "$bad_artifact/ANGLE_RELEASE_MANIFEST.new"
      mv "$bad_artifact/ANGLE_RELEASE_MANIFEST.new" "$bad_artifact/ANGLE_RELEASE_MANIFEST"
      /usr/bin/shasum -a 256 "$bad_artifact/ANGLE_RELEASE_MANIFEST" | awk '{print $1 "  ANGLE_RELEASE_MANIFEST"}' > "$bad_artifact/ANGLE_RELEASE_MANIFEST.sha256" ;;
    chrome-version)
      sed 's/^CHROME_VERSION=.*/CHROME_VERSION=154.0.8037.59/' "$bad_artifact/ANGLE_RELEASE_MANIFEST" > "$bad_artifact/ANGLE_RELEASE_MANIFEST.new"
      mv "$bad_artifact/ANGLE_RELEASE_MANIFEST.new" "$bad_artifact/ANGLE_RELEASE_MANIFEST"
      /usr/bin/shasum -a 256 "$bad_artifact/ANGLE_RELEASE_MANIFEST" | awk '{print $1 "  ANGLE_RELEASE_MANIFEST"}' > "$bad_artifact/ANGLE_RELEASE_MANIFEST.sha256" ;;
    dylib)
...
failure_output="$fixture/post-create-failure-parent/attempt-20260923-120000/output.app"
failure_results="$fixture/post-create-failure-parent/attempt-20260923-120000/results"
...
copy_failure_output="$fixture/copy-before-failure-parent/attempt-20260923-120000/output.app"
copy_failure_results="$fixture/copy-before-failure-parent/attempt-20260923-120000/results"
...
sign_failure_output="$fixture/sign-dry-run-failure-parent/attempt-20260923-120000/output.app"
sign_failure_results="$fixture/sign-dry-run-failure-parent/attempt-20260923-120000/results"
...
# current-only Framework signing remains present and app-level --deep
