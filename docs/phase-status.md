...
| Phase 3B | prepare設計修正済み・実機試験未実施 | artifact検証は実機で成功したが、初回prepareはsource main executableの`codesign --verify --strict`が`resource fork, Finder information, or similar detritus not allowed`でStage 1停止した。test copy、manifest、dylib、signing receiptは未生成である。元Chromeのxattrは変更しない。source側ではこの既知metadata detritusだけを証拠保存付きwarningとして扱い、未知の署名errorはfatalのままとする。`ditto --noextattr --noqtn`後のclean copyではapp/main/Framework/GPU Helperのstrict検証、Google Developer ID、TeamIdentifier、FinderInfo/ResourceFork不在をすべてfatal gateとして、dylib配置より前に検証する。 |
